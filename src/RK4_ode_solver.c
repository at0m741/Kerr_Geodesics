        ...
        for(; lambda < lambda_max;)
        {
            #pragma omp single
            {
                ...
                for (int mu = 0; mu < 4; mu++) {
                    k1[mu] = v[mu];
                    // printf("Initial k1[%d]: %f\n", mu, k1[mu]);  // Ajout d'une impression de débogage
                    ...
                    // printf("Updated k1[%d]: %f\n", mu, k1[mu]);  // Vérification après mise à jour

                    temp_x[mu] = x[mu] + 0.5 * step_size * k1[mu];
                    temp_v[mu] = v[mu] + 0.5 * step_size * k1[mu];
                    // printf("temp_x[%d]: %f\n", mu, temp_x[mu]);  // Vérification après mise à jour
                }
